...
Detect-It-Easy identifies all file formats (PE, ELF, Mach-O, archives,
documents, ...), not just PE. The engine is a thin Go wrapper that shells out
to the prebuilt native Linux `diec` CLI, parses its `--json` output, and stores
a curated result in Elasticsearch under `plugins.exe.diec`.

## Engine

- Backend: Detect-It-Easy v3.21 `diec` (prebuilt native Qt5 Linux binary. The
  Linux build is C++/Qt, not .NET; the .NET description applies to the
  Windows/macOS builds).
- Base image: `ubuntu:20.04` (focal), the exact target the prebuilt binary is
  built for (it links against focal's `libicu66` / `libdouble-conversion` /
